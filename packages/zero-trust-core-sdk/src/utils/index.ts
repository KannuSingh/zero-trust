...
import { Contract, Interface, InterfaceAbi, Provider, ethers} from 'ethers'
...
export const getNonceValue = async (address:string,provider:Provider) => {
  const contract = new Contract(address!, ["function getNonce() view returns(uint256)"],provider)
  let nonceValue = 0;
  try {
    nonceValue = await contract['getNonce']();
